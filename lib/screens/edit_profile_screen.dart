...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  try {
   ...
   if (user != null && mounted) {
    setState(() {
     ...
     _isLoading = false;
     _image = null; // Reset file lokal saat memuat data baru dari server
    });
   } else {
 ...
 Future<void> pickImage() async {
  ...
  if (picked != null) {
   // Di Web, XFile.path adalah URL Blob sementara (bukan path File nyata).
   // Kita akan menggunakan File(picked.path) untuk semua platform, namun 
   // kita harus menggunakan widget yang benar di bawah (ImageProvider).
   setState(() {
    _image = File(picked.path);
   });
   
   // Opsional: Paksa cache untuk dibersihkan
   if (!kIsWeb) {
    try {
 ...
 void saveProfile() async {
  ...
  if (response['success'] == true) {
   ...
   // 🔥 Reload data untuk mengupdate _currentAvatarUrl dan local storage
   await _loadUserData();
   
   if (mounted) {
    // Navigator.pop(context, true); // Biasanya tidak perlu pop jika reload data sudah dilakukan
   }
  } else {
   ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(
  ...
  setState(() {
   _isSaving = false;
   // 🔥 Penting: Reset _image di frontend setelah berhasil disimpan/gagal
   _image = null; 
  });
 }


 // ... (Fungsi _buildShadowedInput dan _inputDecoration tetap sama)
 ...
 // 🔥 FUNGSI BARU: Mendapatkan ImageProvider yang benar
 ImageProvider? _getAvatarProvider() {
  if (_image != null) {
   // 1. Gambar baru dipilih
   if (kIsWeb) {
    // Untuk Web, gunakan NetworkImage dengan path blob XFile
    return NetworkImage(_image!.path);
   } else {
    // Untuk Mobile/Desktop, gunakan FileImage
    return FileImage(_image!);
   }
  } else if (_currentAvatarUrl != null && _currentAvatarUrl!.isNotEmpty) {
   // 2. Gambar dari server
         ...
         children: [
          Container(
           ...
           child: CircleAvatar(
            ...
            // 🔥 Perbaikan: Gunakan _getAvatarProvider()
